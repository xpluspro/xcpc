...
U fac(int k,LL n){// 求 n! mod pk^tk, 返回值 U{ 不包含 pk 的值 ,pk 出现的次数 } 
	...
	if(y){// 求出循环节的答案 
		...
		ans=Pw(ans,y,P[k]);
	}for(i=y*P[k];i<=n;i++) if(i%p[k])ans=ans*i%P[k];// 求零散部分
	U z=fac(k,x);return U{ans*z.x%P[k],x+z.z};
	...
	return CRT();
}LL exLucas(LL n,LL m,int MM){
	if(m<0||m>n)return 0;
	l=0;M=MM;LL jj=M; //求 C(n,m)mod M,M=prod(pi^ki), O(pi^kilg^2n)
	for(LL i=2;i*i<=jj;i++)if(jj%i==0)
