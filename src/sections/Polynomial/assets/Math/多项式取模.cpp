...
// 多项式除法，自动忽略除数 b 的高次补零，要求 b 非零
// n = a.size()，m = b 去掉高次补零后的长度
// n >= m 时商长 n-m+1、余数长 m-1；否则商为空、余数为 a
...
// 多项式取模，同样忽略 b 的高次补零且要求 b 非零，返回 (余数，商)
pair<poly, poly> poly_mod(const poly& a, const poly& b) {
	...
	if (n < m) return {a, {}};
	auto d = poly_div(a, b);
	auto c = poly_auto_mul(poly(b.begin(), b.begin() + m), d);
	poly r(m - 1);
	for (int i = 0; i < m - 1; i++)
		r[i] = (a[i] - c[i] + p) % p;
	return {r, d}; }
