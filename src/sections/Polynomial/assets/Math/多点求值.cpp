struct poly_eval { poly f; vector<int> x; int query_count; // 函数和询问点
	...
	void solve(int l, int r, int o, const poly& rem) {
		if (l == r) { ans[l] = rem[0]; return; }
		int mid = (l + r) / 2;
		solve(l, mid, o * 2, poly_mod(rem, gs[o * 2]).first);
		solve(mid + 1, r, o * 2 + 1,
	...
	vector<int> operator() () { // 包装好的接口
		...
		ans.resize(query_count); return ans;} };
