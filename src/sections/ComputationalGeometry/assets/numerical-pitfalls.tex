\begin{itemize}[leftmargin=5mm]
  ...
  \item 闭半平面可行性判定默认距离容差为 $0$，只有显式传入正容差才放宽边界；
    舍入补偿仍可能合并机器精度量级的间隙，不能代替精确算术。
    已知 $aX+bY\ge c$ 时优先使用
    \texttt{halfplane\_feasible\_closed\_coefficients}，传入法向量 $(a,b)$ 和常数 $c$。
    用两个浮点端点相减重建方向可能破坏原本的平行关系，checker 无法恢复已丢失的信息。
